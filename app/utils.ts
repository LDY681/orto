...
function sortOrderBy(a: number | string, b: number | string, order: string = 'asc') {
  if (order === 'asc') {
    if (a > b) return 1
    if (a < b) return -1
  } else {
 ...
 */
export function sortPostsOrderByKey<T extends MDXDocument>(
  allBlogs: T[],
  key: string = 'title',
  order: 'asc' | 'desc' = 'asc'
) {
 ...
 * @returns <T extends MDXDocument>[]
 */
export function sortPostsByYearAndTitle<T extends MDXDocument>(
  allBlogs: T[],
  order: 'asc' | 'desc' = 'asc'
) {
